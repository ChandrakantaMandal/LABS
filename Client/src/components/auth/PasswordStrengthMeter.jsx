...
export default function PasswordStrengthMeter({ password, isDarkMode = true }) {
  const checks = [
  ...
  const currentStrength = password ? strengthMap[Math.min(score, 4)] : strengthMap[0]
  ...
  return (
    <div className={`mt-2 rounded-xl border px-3 py-2 ${borderClass} ${isDarkMode ? 'bg-slate-900/40' : 'bg-slate-50'}`}>
      <div className="mb-2 flex items-center justify-between text-[11px]">
        ...
        <span className={`font-semibold ${currentStrength.text}`}>{password ? currentStrength.label : 'Not started'}</span>
      </div>
      ...
      <p className={`mb-2 text-[11px] ${mutedText}`}>{password ? currentStrength.hint : 'Use 8+ characters with a mix of letters, numbers, and symbols.'}</p>
